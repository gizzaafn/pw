...
async function saveData(e) {
  ...
  if (id) {
    // Update
    const { error } = await supabase.from('pw').update({ judul, isi }).eq('id', id)
    if (error) return alert('Error update: ' + error.message)
  } else {
    // Insert
    const { error } = await supabase.from('pw').insert([{ judul, isi }])
...
async function deleteData() {
  ...
  const { error } = await supabase.from('pw').delete().eq('id', id)
